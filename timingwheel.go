...
// Wheel 是一个单层分桶时间轮：把任务按「对齐到 Tick 边界的到期时刻」
// 分桶，由单个对齐 ticker 每 Tick 批量执行到期桶中的任务。
//
// 与最小堆定时器相比，Wheel 的排期是 O(1)（直接落桶），代价是到期
...
// NewWheel 创建一个时间轮并启动到期处理循环。tick 是到期检查的粒度，
...
// 每次执行完后按当前时刻重新排期，实际周期 ≈ interval + 单次执行耗时，
// 且不会重叠执行。interval <= 0 时 panic。
...
func (w *Wheel) addTask(due int64, t *Task) {
	w.mu.Lock()
	if due <= w.lastDue {
...
func (w *Wheel) rebucket(t *Task, due int64) {
	next := w.align(due + int64(t.interval))
	w.addTask(next, t)
}

// align 把时刻向上对齐到 Tick 边界，保证落到未来的桶。
func (w *Wheel) align(x int64) int64 {
	tickNs := int64(w.tick)
